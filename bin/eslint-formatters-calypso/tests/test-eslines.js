...
const rulesToNotDowngrade = require( './fixtures/rules-to-not-downgrade.json' );
...
test( 'eslines - errors in files&lines not modified downgraded to warnings, except specific rules to not downgrade', t => {
	const newReport = eslines( baseReport, lines, rulesToNotDowngrade );
...
// eslint-disable-next-line max-len
test( 'eslines - errors in files&lines not modified downgraded to warnings, except specific rules to not downgrade (contains also parsing errors)', t => {
	// ESLint parsing errors have null as the ruleId
	// if we pass it in the array of rules to not downgrade, they would be detected
	const newReport = eslines( parsingErrorsReport, lines, rulesToNotDowngrade );
